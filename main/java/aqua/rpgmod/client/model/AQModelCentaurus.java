...
public class AQModelCentaurus extends AQModelPlayer
{
	private AQHorsePart horsePart = new AQHorsePart();
	
	public AQModelCentaurus()
	{
		super("centaurus", Centaurus.instance());
	}
	...
	public void render(EntityPlayer player, ModelBiped model, float f, AQTextureMap textures, int type)
	{
		...
		if(type == 0)
		{
			...
			AQTextureManager.bindTexture(player, TextureType.HORSE, textures);
			
			GL11.glPushMatrix();
			
			if(player == null)
				GL11.glTranslated(0.0, 11.5, 8.2);
			else
				GL11.glTranslated(0.0, 0.4, 0.48);
			
			this.horsePart.render(f);
			GL11.glPopMatrix();
		}
	}
	...
	public void setRotationAngles(ModelBiped model, float p1, float p2, float p3, float p4, float p5, float p6, Entity entity, int type)
	{
		...
		if(model.onGround > -9990.0F)
		{
			...
			model.bipedRightArm.rotationPointZ = MathHelper.sin(model.bipedBody.rotateAngleY) * 6.0F;
			model.bipedRightArm.rotationPointX = -MathHelper.cos(model.bipedBody.rotateAngleY) * 6.0F;
			model.bipedLeftArm.rotationPointZ = -MathHelper.sin(model.bipedBody.rotateAngleY) * 6.0F;
		...
		if(wrapper != null && wrapper.isSwimming)
		{
			p1 = p3 * 0.5f;
			p2 = 0.4f;
		}
		
		this.horsePart.setLivingAnimations((EntityLivingBase) entity, p1, p2, p6);
	}
}
